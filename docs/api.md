# uniffi-napi API Reference

uniffi-napi is a napi-rs native addon that provides a low-level FFI bridge between Node.js and Rust libraries built with [UniFFI](https://github.com/nickel-lang/uniffi-rs). It replaces ffi-rs with proper support for BigInt, struct-by-value, callbacks, and cross-thread dispatch.

This document is the reference for **code generators** (e.g. uniffi-bindgen-node) that emit JavaScript/TypeScript code targeting uniffi-napi.
...
Closes the underlying library handle. After calling `close()`, all functions returned by `register()` become invalid.
...
The C-level signature is: `uint64_t uniffi_foo_fn_bar(int32_t, RustBuffer, RustCallStatus*)`. The `RustCallStatus*` is invisible to the JS caller — uniffi-napi handles it internally based on `hasRustCallStatus`.
...
- **Main thread:** The JS function is called directly (synchronous)
- **Other thread:** Arguments are serialized and dispatched to the Node.js event loop via `ThreadsafeFunction` (asynchronous for void callbacks, blocking for callbacks with return values)
...
For each `Callback` field, uniffi-napi:
...
This table shows how each variant of UniFFI's `FfiType` enum (from `uniffi_bindgen::interface::FfiType`) maps to a uniffi-napi `FfiType` constant.

| UniFFI `FfiType` | uniffi-napi `FfiType` | Notes |
|------------------|----------------------|-------|
...
| `FfiType::RustCallStatus` | *(not used directly)* | Controlled by `hasRustCallStatus` flag |
...
This section describes how a code generator (e.g. uniffi-bindgen-node) should emit code targeting uniffi-napi.
...
export function myObj_doThing(handle: bigint, arg1: number, buf: Uint8Array): Uint8Array {
  const callStatus = { code: 0 };
  const result = nativeModule.uniffi_mylib_fn_method_myobj_do_thing(handle, arg1, buf, callStatus);
...
| Callback function arguments | Yes | Per-call trampolines via `ffi_closure_alloc` |
| Cross-thread callbacks | Yes | Dispatched via `ThreadsafeFunction` |
| VTable structs (foreign traits) | Yes | Struct of callback function pointers, passed by reference |
| Callback return values | Yes | VTable callbacks can return scalars |
