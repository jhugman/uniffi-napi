...
When a registered function has a `Callback(name)` argument, the JS function passed for that argument is wrapped:

**Same thread (main thread):**
- Call the JS function directly, synchronously
- Return the result immediately to the C caller
- This is the fast path for synchronous trait callbacks invoked from the main thread

**Different thread:**
- Dispatch via `napi_call_threadsafe_function`
- For fire-and-forget callbacks (async continuation): use non-blocking mode, no return value needed
- For blocking callbacks (foreign trait methods): use blocking mode, Rust thread waits until JS returns the result
...
The trampoline uses the same-thread/different-thread dispatch described above.
...
- **Dependencies**: napi-rs, dlopen crate, libffi/libffi-sys, libc
