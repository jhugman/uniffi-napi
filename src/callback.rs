...
/// - [`trampoline_cross_thread`] for serialised dispatch via TSF.
///
/// # Safety
///
...
/// when the event loop drains the TSF queue.
///
/// # Safety
///
...
pub fn raw_arg_to_js(env: &Env, raw_arg: &RawCallbackArg) -> napi::Result<napi::JsUnknown> {
    match raw_arg {
        ...
        RawCallbackArg::RustBuffer(data) => {
            ...
            // only called on the main thread (via the TSF callback).
...
/// - We are on the main thread (this function is called from TSF callbacks).
...
pub unsafe fn c_arg_to_js(
    ...
    match desc {
        ...
        FfiTypeDesc::RustBuffer => {
            ...
            // thread.
            let typedarray = napi_utils::create_uint8array(raw_env, rb.data, rb.len as usize)?;
